#!/usr/bin/env python3
"""Bulk-sync orchestrator SQLite: mark groups as 'done' that are already fully scraped in PG.
...
def main():
    ...
    if mark_done:
        sqlite.executemany(
            "UPDATE scrape_groups SET status='done', last_run_at=? WHERE id=?",
            [(now, gid) for gid in mark_done],
        )
    if mark_skipped:
        sqlite.executemany(
            "UPDATE scrape_groups SET status='skipped', notes='no active players or no valid periods' WHERE id=?",
            [(gid,) for gid in mark_skipped],
        )
    sqlite.commit()
    print("\nDone. SQLite updated.")
    pg.close()
