"""Generate notebook 16_top40_2400_strength_breakdown.ipynb.

Run from project root:
    .venv/bin/python notebooks/_generate_16.py
"""
...
nb16 = [
    ("md", "# 16 — Stärke-Bucket-Auswertung: Top-40-Frauen im 2400+-Fenster\n\n"
           "Aufbau auf Notebook 14/15: Top-40-Frauen nach Jahresend-Elo (`published_rating`, "
           "Dezember 2016–2025), gefiltert auf aktuell **aktive** Spielerinnen → 47 Spielerinnen. "
           "Pro Spielerin wird zusätzlich ihr **Schwellenjahr** bestimmt — das erste Jahr, in dem "
           "ihre Dezember-Elo ≥ 2400 lag — und nur Partien **ab diesem Jahr bis 2025** "
           "(permanent, auch bei späteren Rückgängen unter 2400) fließen in die Analyse ein.\n\n"
           ...
           "`rating_history` zur jeweiligen Periode — bewusst ein anderes Rating-Konzept als die "
           "`published_rating`-Jahresendwerte, die die Kohorte/Schwelle definieren):\n\n"
           "- **schwächer**: `diff < -50`\n"
           ...
           "**Datenarchitektur:** ein einziger Partien-Faktentensor (`games`) trägt alle nötigen "
           "Spalten (`name`, `year`, `strength_bucket`, `opponent_sex`); eine generische "
           "`summarize()`/`build_table()`-Kombination erzeugt daraus je nach übergebenen "
           "Gruppierungsspalten alle Tabellen unten — keine Sonderfälle pro Auswertungsebene, "
           "auch der `Gesamt`-Bucket entsteht per Rollup statt als Extra-Codepfad. "
           "Der Datenwürfel je Spielerin × Jahr ist zu groß für sinnvolle Inline-Anzeige und "
           "wird nur exportiert (spätere Basis für eine interaktive Drilldown-Ansicht)."),
    ("code", "\n".join(BOILERPLATE)),

    ("md", "## Kohorte + Schwellenjahr laden\n\n"
           "Eine Query liefert für jede Spielerin, die je in einem Top-40-Jahresende stand, "
           "ihre komplette Jahresend-Historie 2016–2025 (Rang + Rating) — daraus werden in "
           "Pandas `year_added` (erstes Top-40-Jahr) und `threshold_year` (erstes Jahr mit "
    ...
    ("md", "## Partien-Fakten-Tabelle laden\n\n"
           ...
           "`own_rating` wie in Notebook 13–15 per `LEFT JOIN rating_history` auf `std_rating` "
    ...
    ("md", "## 3. Gesamtzeitraum — pro Spielerin × Bucket × Geschlecht\n\n"
           "Vollständiges Raster (47 Spielerinnen × 4 Buckets [schwächer/gleich/stärker + "
           "**Gesamt** als Summe der drei] × 3 Geschlechts-Gruppen = 564 Zeilen); "
           "Kombinationen ohne Partien erscheinen mit 0."),
    ("code",
    ...
    ("md", "## 4. Gesamtzeitraum — aggregiert über alle 47 Spielerinnen\n\n"
           ...
           "\"schneiden die Top-40-Frauen gegen Männer anders ab als gegen Frauen, über alle "
    ...
    ("md", "## 7. Pro Jahr × Spielerin (Datenwürfel, nur Export)\n\n"
           "Bis zu 47 × 10 × 4 × 3 = 5.640 Zeilen — nicht sinnvoll inline darstellbar. Anders "
           "als Tabelle 3 wird hier **nicht** auf ein volles Raster reindexiert: Jahre vor dem "
           "individuellen Schwellenjahr einer Spielerin fehlen zu Recht (sie waren dort noch "
           "nicht im 2400+-Fenster), eine erzwungene 0-Zeile wäre irreführend. Diese Tabelle ist "
           "die vorgesehene Datenbasis für eine spätere interaktive Drilldown-Ansicht."),
    ("code",
    ...
    ("md", "## 8. Signifikanztest — gleich starke Gegner, Frauen vs. Männer (Spieler-Ebene)\n\n"
           ...
           "0,522 gegen Frauen vs. 0,503 gegen Männer — beide über 50&nbsp;%, aber ist der "
           "*Unterschied* zwischen beiden mehr als Stichprobenrauschen? Partien sind nicht "
           "unabhängig (manche Spielerinnen liefern viele Partien in dieser Zelle, andere kaum "
           "welche), daher wie in Notebook 14 ein **Permutationstest auf Spieler-Ebene** (Ø "
           "Punktequote pro `fide_id`, nicht pro Partie): 10.000 Permutationen, fester Seed, "
           "kein scipy/statsmodels nötig."),
    ("code",
    ...
    ("md", "### Robustheits-Check: Mindestanzahl Partien pro Spielerin\n\n"
           ...
           "eine mit 77 — ihr persönlicher Mittelwert ist dann extrem verrauscht (z.B. 1/6 "
           "Punkten bei nur 3 Partien). Sweep über eine Mindest-Partienzahl pro Spielerin prüft, "
           "ob der Effekt robust ist oder von genau diesen dünn besetzten Zellen getrieben wird."),
    ("code",
    ...
    ("md", "## Export\n\n"
           "CSVs landen (wie bei Notebook 15) relativ im Ausführungsverzeichnis "
           "(`notebooks/`, per `.gitignore` ausgeschlossen). `top40_2400_overall_by_sex.csv` "
    ...
    ("md", "## Fazit\n\n"
           ...
           "Top-40-Frauen im 2400+-Fenster gegen gleich starke/stärkere/schwächere Männer "
           ...
           "Vergleichszelle — gleich starke Gegner, wo das Ø-Gegner-Rating gegen Frauen und "
           "Männer praktisch identisch ist — ob der kleine Punktequote-Unterschied (0,522 vs. "
           "0,503) über Stichprobenrauschen hinausgeht, oder ob die beiden Gruppen "
           "statistisch nicht unterscheidbar sind.\n\n"
           "**Caveats:**\n"
           "- Bucket-Grenzen: `gleich` schließt exakt ±50 Elo ein (wie in Notebook 13/14); "
           "`schwächer`/`stärker` beginnen erst echt darüber/darunter; `Gesamt` ist die reine "
           "Summe der drei (keine eigene Stärke-Definition).\n"
           ...
           "- Das Schwellenjahr-Fenster ist pro Spielerin unterschiedlich lang (2016–2025 vs. "
           "z.B. nur 2025) — Aggregat-Tabellen über alle Spielerinnen sind entsprechend nicht "
           "gleichgewichtig über die Karrierephasen verteilt."),
]


if __name__ == "__main__":
    make_notebook(NBDIR / "16_top40_2400_strength_breakdown.ipynb", nb16)
